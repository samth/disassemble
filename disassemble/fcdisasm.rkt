...
(define (disassemble p disassembler color end-position pc symbols relocations
                     is-ip? is-jump? pc-rel-delta)
  ...
  (let ([jump-targets (get-jump-targets p get-instruction is-ip? is-jump? pc-rel-delta end-position pc)]
      ...
      (let* ((tagged-bytes '())
        ...
        (unless (or (eof-object? i)
          ...
          (cond
           ...
           [else
            (let ([new-sync-points (extract-sync-points new-pc i end-pc is-ip? pc-rel-delta)])
...
(define (get-jump-targets p get-instruction is-ip? is-jump? pc-rel-delta end-position start-pc)
...
(define (extract-sync-points pc i end-pc is-ip? pc-rel-delta)
  (let loop ([i i])
    (cond
     [(null? i) '()]
     [else
